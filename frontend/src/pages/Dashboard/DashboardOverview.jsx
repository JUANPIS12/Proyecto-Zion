...
import { RefreshCw, ClipboardList, CheckCircle2, Users, MapPin, Wrench, Settings, Globe, ShieldCheck, LayoutDashboard } from 'lucide-react';
...
export default function DashboardOverview() {
  ...
  const resentOrdersMap = useMemo(() => {
    return ordenes.slice(0, 5).map((orden) => ({
      ...
      cliente: orden.empresaNombre || `Empresa #${orden.empresaId ?? ''}`,
      estado: orden.estado,
      tecnico: orden.tecnicoNombre || `Técnico #${orden.tecnicoId ?? ''}`,
      sede: orden.sedeNombre || '',
    }));
  }, [ordenes]);

  // Si el usuario es técnico, mostrarle su propio dashboard personalizado
      ...
      <header className="mb-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-slate-500 font-medium text-lg">
            {user?.rol === 'ROLE_ADMIN'
              ? `Bienvenido, Administrador — Control total del sistema`
              : `Panel de coordinación — ${new Date().toLocaleDateString('es-CO', { weekday: 'long', day: 'numeric', month: 'long' })}`}
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          <button onClick={loadData} className={`${primaryButtonClass} flex items-center gap-2`}>
        ...
        {stats.map((stat) => (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="h-1.5 w-1.5 rounded-full bg-slate-300 group-hover:bg-blue-400 group-hover:animate-pulse transition-soft" />
            </div>
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-3">
              ...
              <tbody>
                {resentOrdersMap.map((order) => (
                  <tr key={order.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-soft">
                    ...
                    <td className="py-4 whitespace-nowrap max-w-[200px] truncate">
                      <div className="font-medium text-slate-900">{order.cliente}</div>
                      {order.sede && <div className="text-xs text-slate-400">{order.sede}</div>}
                    </td>
                    ...
                    <td className="py-4 whitespace-nowrap">
                      <button onClick={() => verDetalleOrden(order.id)} className={smallDarkButtonClass}>
                        Ver detalle
                      </button>
                    </td>
                  </tr>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center pb-2 border-b border-slate-100">
              <span className="text-sm font-semibold text-slate-600">Equipos registrados</span>
              <span className="text-sm font-bold text-slate-900">{equipos.length}</span>
            </div>
            <div className="flex justify-between items-center pb-2 border-b border-slate-100">
              <span className="text-sm font-semibold text-slate-600">Órdenes en proceso</span>
              <span className="text-sm font-bold text-amber-600">{ordenes.filter(o => o.estado === 'EN_PROCESO').length}</span>
            </div>
          </div>
        </div>
      </section>
